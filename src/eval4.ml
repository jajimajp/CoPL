...
and e =
  ...
  | Nil
  | Cons of e * e
  | Match of e * e * e * e * e (* e1, e2, x, y, e3 *)
...
let rec eval (env, e) =
  ...
  | Nil -> NilV
  | Cons (e1, e2) -> ConsV (eval(env, e1), eval(env, e2))
...
let rec solve j =
  ...
  | Evalto (env, e, v) ->
      ...
      | Match (e1, e2, Var x, Var y, e3) ->
          let e1val = eval(env, e1) in
          (match e1val with
          | NilV -> Node ("E-MatchNil", j, [
              solve(Evalto(env, e1, e1val));
              solve(Evalto(env, e2, v));
            ])
          | ConsV (v1, v2) -> Node ("E-MatchCons", j, [
              solve(Evalto(env, e1, e1val));
              solve(Evalto((y, v2)::(x, v1)::env, e3, v));
            ]))
...
let rec e_to_string e =
  ...
  | App (e1, e2) ->
      "(" ^ e_to_string e1 ^ " " ^ e_to_string e2 ^ ")"
  | Nil -> "[]"
  | Cons (e1, e2) -> "(" ^ e_to_string e1 ^ " :: " ^ e_to_string e2 ^ ")"
  | Match (e1, e2, Var x, Var y, e3) ->
      "(match " ^ e_to_string e1 ^ " with [] -> " ^ e_to_string e2
        ^ " | " ^ x ^ "::" ^ y ^ " -> " ^ e_to_string e3 ^ ")"
let reverse lst =
...
let rec v_to_string v =
  ...
  | RecV (env, x, y, e0) ->
      "(" ^ env_to_string env ^ ")[rec "
        ...
        ^ e_to_string e0 ^ "]"
  | NilV -> "[]"
  | ConsV (v1, v2) -> "(" ^ v_to_string v1 ^ " :: " ^ v_to_string v2 ^ ")"
  ...
  LetRec(
    ...
    App(
      App(
        ...
        Cons(
          Fun(Var "x",
            Op(TimesOp, Var "x", Var "x")
          ),
          Cons(
            Fun(Var "y",
              Op(PlusOp, Var "y", Int 3)
            ),
            Nil
          )
        )
